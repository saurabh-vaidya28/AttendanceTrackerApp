...
public class underGraduateLectureHOD extends AppCompatActivity implements AdapterView.OnItemSelectedListener,View.OnClickListener{

    private TextView _lec_ug_hodID,_lec_ug_hodName;
    ...
    ImageView _qrCode;
    private FirebaseAuth firebaseAuth_lec_ug_hod;
    DatabaseReference databaseReference_hod,databaseReference1_hod,databaseReference_lec_ug_hod;
    LectureHOD lectureHOD;
    private ProgressBar _progressBarUG_hod;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_under_graduate_lecture_hod);
        setTitle("Lecture Instance");

        _lec_ug_hodID=(TextView)findViewById(R.id.lec_hodID_ug);
        _lec_ug_hodName=(TextView)findViewById(R.id.lec_hodName_ug);


        _progressBarUG_hod=(ProgressBar)findViewById(R.id.progressBarUGHOD);
...
//        _semester = (SearchableSpinner)findViewById(R.id.semester);
        ...
        databaseReference_hod= FirebaseDatabase.getInstance().getReference().child("Lecture-HOD").child("Under-Graduate");
        databaseReference1_hod= FirebaseDatabase.getInstance().getReference().child("Lecture_Combined-HOD").child("Under_Graduate");

        firebaseAuth_lec_ug_hod= FirebaseAuth.getInstance();
        databaseReference_lec_ug_hod=FirebaseDatabase.getInstance().getReference().child("HOD_Details").child(firebaseAuth_lec_ug_hod.getUid());
        databaseReference_lec_ug_hod.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                String lec_jc_Code_hod=dataSnapshot.child("empCode_of_HOD").getValue().toString();
                _lec_ug_hodID.setText(lec_jc_Code_hod);
                String lec_jc_Name_hod=dataSnapshot.child("name_of_HOD").getValue().toString();
                _lec_ug_hodName.setText(lec_jc_Name_hod);
            }
...
//        _subjectDisplay.setText(_subCodeName.getSelectedItem().toString());
    ...
    private void Generate() {
        ...
        String hodID = _lec_ug_hodID.getText().toString();
        String hodName = _lec_ug_hodName.getText().toString();
        String s_c_n = (String) _subCodeName.getSelectedItem();

        final String text = hodID + "\n" + hodName + "\n" + s_c_n;
        ...
        lectureHOD.setHodID(_lec_ug_hodID.getText().toString().trim());
        lectureHOD.setHodName(_lec_ug_hodName.getText().toString().trim());
        ...
        _progressBarUG_hod.setVisibility(View.VISIBLE);
        if (text != null && !text.isEmpty()) {
            try {
                databaseReference1_hod.child(firebaseAuth_lec_ug_hod.getUid()).push().setValue(lectureHOD);
                _progressBarUG_hod.setVisibility(View.GONE);
            ...
            try {
                databaseReference_hod.child(_department.getSelectedItem().toString())
                        .child(_semester.getSelectedItem().toString()).child(firebaseAuth_lec_ug_hod.getUid()).push().setValue(lectureHOD)
