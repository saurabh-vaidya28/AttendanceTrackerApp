...
public class HODLectureAdapter extends FirebaseRecyclerAdapter<LectureHOD, HODLectureAdapter.PastViewHolder> {

    public HODLectureAdapter(@NonNull FirebaseRecyclerOptions<LectureHOD> options) {
        super(options);
    }
...
*/
    ...
    @Override
    protected void onBindViewHolder(@NonNull PastViewHolder holder, int position, @NonNull LectureHOD lectures) {
        ...
        if(lectures.getHodID() != null && !lectures.getHodID().isEmpty()) {
            letter = lectures.getHodID().substring(0, 1);
        }
    }
